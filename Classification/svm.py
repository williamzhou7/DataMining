...
def load_dataset(dataset='cancer'):        
    if dataset == 'carro':
        # Load carro data and store in dataframe
        carro = datasets.load_carro()
        names = carro.target_names
        df = pd.DataFrame(data=carro.data, columns=carro.feature_names)
        df['target'] = carro.target
    elif dataset == 'cancer':
...
def main():
    #load dataset
    target_names, df = load_dataset('carro')
